...
impl RecordsGroup {
    ...
    pub(crate) fn split(self, max_items: NonZeroU64) -> Result<Self> {
        let max_items_u64 = max_items.get();
        ...
        Ok(match self {
            ...
            RecordsGroup::Profiles(items) => {
                for batches in &items {
                    let count = num_items(batches);
                    if count as u64 > max_items_u64 {
                        return Err(Error::TooManyItems {
                            payload_type: ArrowPayloadType::Profiles,
                            count,
                            max: max_items_u64,
                            message: "Profiles batches cannot be split until graph-aware partitioning is implemented"
                                .to_string(),
                        });
                    }
                }
    ...
    pub(crate) fn concatenate(self, max_items: Option<NonZeroU64>) -> Result<Self> {
        Ok(match self {
            ...
            RecordsGroup::Profiles(items) => RecordsGroup::Profiles(items),
        })
    }
