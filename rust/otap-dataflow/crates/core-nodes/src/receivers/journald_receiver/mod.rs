...
//! configuration types, per-namespace process-local lease, and a no-op
...
pub use config::{
    ...
    StartAt, TransientErrorConfig, severity_number_from_priority,
};
...
pub static JOURNALD_RECEIVER: ReceiverFactory<OtapPdata> = ReceiverFactory {
    ...
    create: create_journald_receiver,
