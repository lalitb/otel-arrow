...
impl<PData> EffectHandlerCore<PData> {
    ...
    ///
    /// Returns the scheduler-assigned [`LocalResumeId`] so callers can correlate
    /// the eventual `DelayedData` delivery with per-payload state kept out of
    /// band (such as a `!Send` memory-budget ticket).
