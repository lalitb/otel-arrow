...
The `kernel` strategy leaves selection to the default Linux reuseport hash and
preserves the existing independent bind path. The `ebpf_numa` strategy enables
controller listener-group planning and attaches the selector described here when
the binary was built with selector support. The `engine` strategy is accepted as
a reserved value for a future controller-owned policy; current implementations
must report that it is not implemented and leave listeners on the independent
bind path.
...
The user-facing model is the engine policy above. The controller resolves the
policy once with the rest of pipeline configuration and passes the result to
placement, listener planning, and receiver runtime handles. Listener paths do
not read process-global activation variables.

The eBPF loader itself sits behind a `reuseport-ebpf` Cargo feature so the
default build carries no BPF toolchain or runtime dependency. With the policy
set to `kernel`, behavior is identical to today: every receiver binds
independently and no coordination, planning, or attach occurs.

When `strict` is true for an `ebpf_numa` group, selector setup failure aborts
startup. When it is false, selector setup failure falls back to the existing
independent bind path.
