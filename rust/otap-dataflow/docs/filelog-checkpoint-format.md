...
**Status:** Implementation-ready draft (Phase 1, Stage 2), pending maintainer
approval. This is the exact, implementation-ready version-1 byte encoding
referenced by
[`filelog-receiver.md`](filelog-receiver.md) Appendix B. Appendix B defines the
logical model (namespace, snapshot/WAL split, durable state transitions, logical
WAL operations, recovery algorithm, framing-profile compatibility contract).
This document defines everything needed to encode and decode those logical
entities as bytes: magic values, versions, byte order, fixed widths, field
order, maximum encoded lengths, checksum coverage, and every persisted
discriminant.
...
- `ownership.lock` is an empty lock file used only for advisory OS locking
  (POSIX `flock`/`fcntl` or Windows `LockFileEx`, per Appendix B D15); it has
  no format of its own and is out of scope for this document.
...
(Appendix B's compaction algorithm); a reader never observes a snapshot that
is genuinely still being written. Therefore any of the following is
corruption and fails recovery closed, with no leniency, even if it is the
...
the (later-stage) runtime is responsible for populating from the current
platform.
...
| `0x02` | `WindowsVolumeFileId` | `volume_serial: u32` BE, `file_id: [u8; 16]` opaque | 21 bytes |
...
- **`Unspecified`** represents "no runtime locator recorded" -- for example,
  ...
  Appendix B requires this case to be defined and tested rather than
  silently substituting a fallback identity; `Unspecified` is that explicit,
  normalized representation, and higher-level policy (a later stage) decides
  what a receiver does when it encounters this value.
...
- **`WindowsVolumeFileId`** normalizes `(volume_serial, FILE_ID_INFO)`.
  `volume_serial` is the 32-bit `DWORD` volume serial number. `file_id` is
  the 128-bit `FILE_ID_INFO.FileId` byte array, copied verbatim in the byte
  order the platform API already returns (it is an opaque 16-byte
  identifier, not an integer, so no additional byte-order conversion is
  applied by this format).
...
`Clean` is INV-FR1's common resume state: the next complete source unit
...
| `0x0004` | framing-profile version/digest mismatch quarantine |
| `0x0005`-`0x00FF` | reserved for future built-in reasons |
...
- `op_count == 0` is rejected: every transaction carries at least one
  ...
  matches Appendix B's statement that a transaction "contains a bounded set
  of operations."
...
definition an operator-authorized administrative action (Appendix B: "the
operator-authorized administrative reset path").
...
layers (a later implementation stage) additionally bound the practical
number of tracked files and the practical transaction size through
configuration (`limits.max_tracked_files`, checkpoint sync/compaction
policy); this document does not duplicate those runtime policies as format
constants.
...
- `action == reset_to_end` (`0x02`): `resulting_epoch` MUST equal
  ...
  replacement stream's actual EOF; supplying a correct value is a runtime
  responsibility documented for a later implementation stage).
...
The recovery algorithm (Appendix B) discards **only** a structurally
incomplete final transaction and fails closed on every other error. This
document defines "structurally incomplete" precisely, so the distinction is
...
Appendix B requires each checkpoint to store a framing-profile version and a
digest covering "all configuration affecting record boundaries or
deterministic replay," with a version or digest mismatch against resumable
state failing closed. This section defines the exact canonical serialization
fed to the digest and the digest algorithm, so two independent
implementations produce byte-identical digests for the same configuration.
...
`format_version` field: this version tracks only the digest recipe below,
and can in principle advance on its own if the set of configuration
parameters that affect framing changes, without requiring a change to the
snapshot/WAL byte layout.
...
This list is exactly the Appendix C framing knobs that change record
boundaries or replay determinism (`encoding`, the multiline mode and
...
knobs that do not affect framing determinism, such as `checkpoint.id`,
...
fixed ASCII prefix (including the trailing NUL) used by the fragment-ID
construction in `filelog-receiver.md`, so the two digests can never collide
with each other by construction even if their remaining field shapes were
ever to coincide.

### Compatibility vectors

Given the newline-framing default profile (`encoding = utf-8 (0x01)`,
...
`force_flush_period_millis = 500`), the canonical serialization is 73 bytes
(38-byte domain-separated prefix + 1 + 1 + 2 + 2 + 0 + 8 + 8 + 1 + 4 + 8) and:

```text
framing_profile_digest = f00ca1eef473e3dc0dbd141e378270c0be2e6d698a4603d8bb63c81acbeed537
```
...
86-byte canonical input and digest:

```text
framing_profile_digest = 5343f5c8ebe99bf88cb1663791aa62d10da364f6da581bbd556a5cfa11087d22
```
...
- **`remove_file`** (`0x08`) is conditionally administrative, distinguished
  ...
  path capable of removing a `Quarantined` record, matching Appendix B's
  "removing it requires an explicit administrative operation naming the
  exact `checkpoint.id` and `file_id` with an audit reason."
...
Executable golden/conformance vectors live in
`crates/core-nodes/src/receivers/filelog_receiver/checkpoint/test_vectors.rs`
and are consumed by tests in
`crates/core-nodes/src/receivers/filelog_receiver/checkpoint/tests.rs`. They
are independently computed (using a separately verified CRC-32C
implementation and standard SHA-256, not by round-tripping through this
crate's own encoder) and cover:
...
corresponding round-trip/corruption/torn-write/cross-version/cross-platform/
